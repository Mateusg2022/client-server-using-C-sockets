...
void* client_thread(void *data){
    ...
    char buf[BUFSZ];
    memset(buf, 0, BUFSZ);
    size_t count = recv(cdata->csock, buf, BUFSZ, 0); //no servidor, vamos receber todos os dados de uma vez. se o client mandasse em pacotes, deveria ter a mesma logica de receber bytes ate chegar em BUFSZ bytes
...
int main(int argc, char **argv){
    ...
    while(1){
        ...
        //aceitou --> cria thread
        pthread_t tid;
        pthread_create(&tid, NULL, client_thread, cdata); //paraleliza a comunicação com o cliente criando uma thread para a funcao client_thread. Alias, a funcao pthread_create exige que a funcao seja receba *void e retorne void*
    }
